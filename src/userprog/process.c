...
/* Starts a new thread running a user program loaded from
   ...
   thread id, or TID_ERROR if the thread cnnot be created. */
...
//for debugging
	...
	for (child_list_elem = list_begin(&list_child); 
		...
		//printf("tmp_child tid : %d\n child_tid : %d\n",tmp_childt->tid, child_tid);
		//printf("child candidate name:%s || tid:%d:\n",\
			tmp_childt->name,(int)(tmp_childt->tid));
		//현재 thread : parent의 child중 파라미터로 받아온 tid값과 같은 값을 갖는 child thread찾기.
    ...
    if(tmp_childt->tid < 0){
       return -1;
		...
		//printf("no child name:%s || tid:%d || exit_status:%d\n", \
			tmp_childt->name, (int)(tmp_childt->tid), (int)(tmp_childt->exit_status));
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  //before open file, parse filename
  argv[0] = strtok_r(file_name, " ", &next_ptr);//https://jybaek.tistory.com/593, strtok_r is more thread-safe function. strtok:using own static variable old, strtok_r(s,&delim,old).
  ...
  real_file_name = argv[0];//https://jybaek.tistory.com/593, strtok_r is more thread-safe function. strtok:using own static variable old, strtok_r(s,&delim,old).
  ...
  if (!setup_stack (esp))
    goto done;
  //printf("in load(), after setup_stack()\n");
  //printf("BASE : %u\n", (uint32_t)(*esp));
  uint32_t startaddr = (uint32_t)(*esp);//for debugging
  uint32_t addr_of_argv[MAX_TOKEN_NUM];//do i need optimization??
  int arg_total_len = 0;
  
  //printf("in load(), argv[3][], argv[2][], ....\n");
  for (i=argc-1; i >= 0; i--) {
	...
	//printf("%u %s\n", addr_of_argv[i], argv[i]);
  }

  int wordalign = 0;

  //wordalign
  //printf("in load(), word align\n");
